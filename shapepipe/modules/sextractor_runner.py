...
def make_post_process_wo_sex(tile_path, 
                             cat_path,
                             f_wcs_path,
                             pos_params,
                             ccd_size,
    ...
    ----------
    tile_path : str
        Path to tile corresponding to the catalog
    cat_path : str
        Path to the outputed SExtractor catalog
    f_wcs_path : str
        Path to the log file containing wcs for all single exp CCDs
    ...
    tile_header = fits.getheader(tile_path, 0)

    data = np.loadtxt(cat_path)
    cat_txt = open(cat_path)
    tmp = re.split('\s+', cat_txt.readline())
    col_name = [] 
    while tmp[0] == '#':
        col_name.append(tmp[2]) 
        tmp = re.split('\s+', cat_txt.readline())

    cat = io.FITSCatalog(output_file_path,
                         open_mode=io.BaseCatalog.OpenMode.ReadWrite)

    # Has to be improve
    cat.save_as_fits(data=np.array([1]), names=['empty'], ext_name='EMPTY')

    # Create SEx_cat
    cat.save_as_fits(data=data.T, names=col_name, ext_name='LDAC_OBJECTS')
    cat.hdu_no = 2
    cat.open()

    # pos_params = np.array(pos_params).astype(int)
    f_wcs = SqliteDict(f_wcs_path)
    n_hdu = len(f_wcs[list(f_wcs.keys())[0]])

    exp_list = []
    pattern = r'([0-9]*)p\.(.*)'
    for i in tile_header['HISTORY']:
        m = re.search(pattern, i)
        exp_list.append(m.group(1))
...
def sextractor_runner(input_file_list, run_dirs, file_number_string,
    ...
    run_sex = config.getboolean("SEXTRACTOR_RUNNER", "RUN_SEXTRACTOR")
    ...
    if run_sex:
        ...
        if not config.getboolean("SEXTRACTOR_RUNNER", "MAKE_POST_PROCESS"):
            raise ValueError('SExtractor is not run and no post-process ' 
                             'requiered. Either or both has to be set on True.')

        tile_path = input_file_list[0]
        catalog_path = input_file_list[1]
        f_wcs_path = config.getexpanded("SEXTRACTOR_RUNNER", "LOG_WCS")
        pos_params = config.getlist("SEXTRACTOR_RUNNER", "WORLD_POSITION")
        ccd_size = config.getlist("SEXTRACTOR_RUNNER", "CCD_SIZE")
        make_post_process_wo_sex(tile_path, 
                                 catalog_path,
                                 f_wcs_path,
                                 pos_params,
                                 ccd_size,
