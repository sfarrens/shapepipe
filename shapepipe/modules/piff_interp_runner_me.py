...
def piff_interp_runner_me(input_file_list, run_dirs, file_number_string,
                           config, w_log):

    mode = config.get('PIFF_INTERP_RUNNER_ME', 'MODE')

    pos_params = config.getlist('PIFF_INTERP_RUNNER_ME', 'POSITION_PARAMS')
    get_shapes = config.getboolean('PIFF_INTERP_RUNNER_ME', 'GET_SHAPES')
    star_thresh = config.getint('PIFF_INTERP_RUNNER_ME', 'STAR_THRESH')
    chi2_thresh = None
    ...
    if mode == 'CLASSIC':
        ...
        dot_psf_pattern = config.get('PIFF_INTERP_RUNNER_ME', 'ME_DOT_PSF_PATTERN')
        f_wcs_path = config.getexpanded('PIFF_INTERP_RUNNER_ME', 'ME_LOG_WCS')
