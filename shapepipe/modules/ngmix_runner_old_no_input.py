...
def do_ngmix_metacal(prior, id_tmp, rng):
    """ Do ngmix metacal
...
def process(w_log, id_obj_min=-1, id_obj_max=-1,
...
def ngmix_runner_old_no_input(input_file_list, run_dirs, file_number_string,
                 config, w_log):
    ...
    ZP = config.getfloat('NGMIX_RUNNER_OLD_NO_INPUT', 'MAG_ZP')

    # f_wcs_path = config.getexpanded('NGMIX_RUNNER_OLD', 'LOG_WCS')

    id_obj_min = config.getint('NGMIX_RUNNER_OLD_NO_INPUT', 'ID_OBJ_MIN')
    id_obj_max = config.getint('NGMIX_RUNNER_OLD_NO_INPUT', 'ID_OBJ_MAX')

    metacal_res = process(w_log,
